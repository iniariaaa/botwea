...
handler.all = async function (m, { isPrems, isOwner }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        ...
        await this.sendFile(m.chat, json.wm, '', '© AriaBotz', m)
    }
    ...
    if (/^.*(fb.watch|facebook.com)/i.test(m.text)) {
        facebook(m.text.split` `[0]).then(async res => {
            ...
            await m.reply(global.wait)
            await this.sendFile(m.chat, json.data[1] != undefined ? json.data[1].url : json.data[0].url, '© AriaBotz', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(m.text.split` `[0]).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), `© AriaBotz`, m, 0, buf)
            }
        }).catch(_ => _)
    }
...
*Server y2mate:* ${usedServer}
          `.trim(),
            thumb, '© AriaBotz', 'AUDIO', `.yta ${vid.url}`, 'VIDEO', `.yt ${vid.url}`)
    }
