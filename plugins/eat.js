...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode }) =>  {
    ...
    let apel = global.db._data.users[m.sender].apel
    let jeruk = global.db._data.users[m.sender].jeruk
    let mangga = global.db._data.users[m.sender].mangga
    let stroberi = global.db._data.users[m.sender].stroberi
    let semangka = global.db._data.users[m.sender].semangka
    let pisang = global.db._data.users[m.sender].pisang
    ...
    let eatpi = (pisang == 0 ? 40 : '' || pisang == 1 ? 45 : '' || pisang == 2 ? 50 : '' || pisang == 3 ? 55 : '' || pisang == 4 ? 60 : '' || pisang == 5 ? 65 : '' || pisang == 6 ? 70 : '' || pisang == 7 ? 75 : '' || pisang == 8 ? 80 : '' || pisang == 9 ? 85 : '' || pisang == 10 ? 90 : '')
        let healt = global.db._data.users[m.sender].healt
        if (/eat|makan/i.test(command)) {
            try {
            count = (/[0-9]/g.test(args[1])) ? !args[1] || args.length < 2 ? Math.max((Math.ceil((100 - global.db._data.users[m.sender].healt) / eatap)), 1) : Math.max(args[1], 1) : Math.max((Math.ceil((100 - global.db._data.users[m.sender].healt) / eatap)), 1)
                 let msgsucces = (pickRandom(['Sukses Memakan', 'Nice Sukses Memankan', 'Berhasil Memakan', 'Bagus Anda Memakan', 'Anda Memakan', 'Anda Memakan']) + ' *' + (count * 1) + '* Apel')
                                  if (args[0] === 'apel') {
                    if (global.db._data.users[m.sender].healt < 100) {
                        if (global.db._data.users[m.sender].apel >= count * 1) {
                            global.db._data.users[m.sender].apel -= count * 1
                            global.db._data.users[m.sender].healt += eatap * count
